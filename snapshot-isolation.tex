...
\begin{frame}
	Informally,
	\begin{itemize}
		\item $T \xrightarrow{WR_\mathcal{X}(x)} S $ means that $S$ reads $T$'s write to $x$.
		\item $T \xrightarrow{WW_\mathcal{X}(x)} S $ means that $S$ overwrites $T$'s write to $x$.
		\item $T \xrightarrow{RW_\mathcal{X}(x)} S $ means that $S$ overwrites the write to $x$ read by $T$.
	\end{itemize}
\end{frame}
...
\begin{frame}
	\begin{definition}
		...
		\pause
		\item<3> RW: $Obj \rightarrow 2^{\mathcal{T} \times \mathcal{T}}$ is derived from WR and WW as in the definition of $RW_\mathcal{X}(x)$.
		\end{itemize}
	\end{definition}
\end{frame}
