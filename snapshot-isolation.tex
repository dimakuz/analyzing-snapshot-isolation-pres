...
\begin{frame}
\begin{definition}
	A \textbf{dependency graph} is a tuple $\mathcal{G} = (\mathcal{T}, SO, WR, WW, RW)$, where $(\mathcal{T}, SO)$ is a history and for each $x \in Obj$ we define the following relations:
	\begin{itemize}
		\item \textbf{read dependency} $WR(x)$ as:
		\begin{itemize}
		...
		\end{itemize}
		\item \textbf{write dependency} $WW(x)$ is a total order over $WriteTx_x$.
		\item \textbf{anti dependency} $RW(x)$ is derived from $WR(x)$ and $WW(x)$ such that $T \xrightarrow{RW(x)} S \Leftrightarrow
